...
parser.add_argument("-o",choices=['union', 'intersect', 'diff', 'symmdiff','all'], required=True)
...
print(f'{script_dir} {path1} {path2} {choice}')
...
def unions(file1,file2):
    ...
    with open('union.txt','a') as f:
        resultunion = sorted(set1.union(set2))
...
def intersection(file1,file2):
    ...
    with open('intersection.txt','a') as f:
        resultintersection = sorted(set1.intersection(set2))
...
def symm_diff(file1,file2):
    ...
    with open('symmetric_diff.txt','a') as f:
        resultsymmdiff = sorted(set1.symmetric_difference(set2))
...
def difference(file1,file2):
    ...
    with open('difference_file1_to_file2.txt','a') as f:
        resultsdifference1 = sorted(set1.difference(set2))
    ...
    with open('difference_file2_to_file1.txt','a') as f:
        resultsdifference2 = sorted(set2.difference(set1))
        print('Items after difference file 2 - file 1: ' + str(len(resultsdifference2)))
        for line in resultsdifference2:
            f.write(line+' \n')
print ()
print ('UIDS: Union, Intersection, Difference, & Symmetric Difference of 2 text files.')
print ('By: @Aditya Kaushal')
print ("for: @Amazon python intermidiate training")
